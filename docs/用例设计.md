...
| UC-A04 | 订单监控与详情查看 | 发现异常订单/履约问题 | `pages/admin/AdminOrderMonitorPage.vue` |
...
- **UC-A04/UC-A05 订单监控与运营工具**
  - 订单监控调用 `/orders`（支持用户/厂商/状态过滤），抽屉展示 `fetchOrder` 详情。
  - 运营工具箱提供 `previewOrder`、`createOrder`、`shipOrder`、`confirmOrderReceive`、`initPayment`、`confirmPayment`、`sendNotification` 等入口，便于联调与诊断。
