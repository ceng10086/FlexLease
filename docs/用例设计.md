...
| UC-A05 | 查看系统公告 | 了解平台通知动态 | `pages/overview/OverviewPage.vue` 通知卡片 |
...
- **UC-U04 订单试算与创建**
  ...
  3. 平台会在订单创建后立即调用 `/payments/{orderId}/init` 并自动确认支付，若自动支付失败可在订单详情重新触发。

- **UC-U05/UC-U06 订单管理**
  - 列表 `/orders?userId=...` 展示所有订单，支持状态过滤。
  - 详情页调用 `/orders/{id}` 并提供续租 `/extend`、退租 `/return`、买断 `/buyout`、确认收货 `/confirm-receive` 等入口。
  - 支付入口仍调用 `/payments/{orderId}/init`，服务会立即自动确认支付并推送通知。
- **UC-U09 合同查看与签署**
  - 首次进入详情时调用 `/orders/{id}/contract` 自动生成电子合同草稿，可反复刷新确保内容与订单同步。
