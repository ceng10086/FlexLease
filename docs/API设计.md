...
| POST | `/admin/orders/{orderId}/disputes/{disputeId}/ai-suggestion` | ADMIN | 生成“仲裁建议”（只读，不改变纠纷状态） | 请求体 `{ tone?, force? }`；`force=true` 可跳过缓存强制重算；返回结构化建议（事实摘要/缺失证据/建议裁决/通知话术）；未配置外部 LLM Key/网络时返回离线模板建议（用于演示与排查） |
