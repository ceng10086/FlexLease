...
- **时间格式**：ISO 8601（带时区偏移），示例 `2025-01-15T08:00:00+08:00` 或 `2025-01-15T00:00:00Z`。
...
| POST | `/admin/orders/{orderId}/disputes/{disputeId}/ai-suggestion` | ARBITRATOR | 生成“仲裁建议”（只读，不改变纠纷状态） | 请求体 `{ tone?, force? }`；`force=true` 可跳过缓存强制重算；返回结构化建议（事实摘要/缺失证据/建议裁决/通知话术）；未配置外部 LLM Key/网络时返回离线模板建议（用于演示与排查） |
| POST | `/admin/orders/{orderId}/disputes/{disputeId}/resolve` | ARBITRATOR/REVIEW_PANEL | 平台裁决 / 申诉复核裁决 | `{ decision, penalizeUserDelta?, remark? }`；`penalizeUserDelta` 为信用扣分，正数代表扣减（PENDING_ADMIN 由 ARBITRATOR 处理；PENDING_REVIEW_PANEL 由 REVIEW_PANEL 处理；平台管理员无权限） |
...
- 当前未接入 SpringDoc 自动生成 OpenAPI 文档；如需对外提供标准化接口说明，可在后续迭代引入并补充到 `docs/`。
