...
- `backend/order-service`：新增 `OrderProofServiceIntegrationTest`（多角色取证上传/下载、站内信提醒）、`OrderSurveyServiceIntegrationTest`（满意度调研调度与提交）、`RentalOrderServiceLazyLoadingTest` 和 `OrderQueryLazyLoadingTest`（校验 Web 层读取订单时不会触发懒加载异常），并通过 `OrderServiceApplicationTests` 覆盖基础启动与配置。
...
- “生成仲裁建议”默认可用：未配置外部 LLM 时会返回离线模板化建议（用于演示与 E2E）；如需接入外部 LLM，参考 `.env.example` 与 `docs/纠纷仲裁智能助手.md` 开启并填写 Key。
...
- **订单（已覆盖）**：已新增订单全流程集成测试，覆盖下单→支付→发货→续租→退租闭环，并校验买断失败场景；管理员分支增加强制关闭、分页检索等回归用例。
...
- **支付/通知（已补充）**：支付服务校验退款窗口与分账，通知服务写入站内信日志，后续可按需补充失败重试测试。
...
- 指标目标：前后端核心模块单测覆盖 ≥ 60%，关键业务（订单、支付）分支覆盖 ≥ 80%。
...
- 性能与安全：后续迭代准备压测基线与 OWASP ZAP 被动扫描，同时在订单模块通过 `RentalOrderServiceLazyLoadingTest`/`OrderQueryLazyLoadingTest` 确认懒加载安全。

## 6. 监控与日志
- 日志：默认使用 Spring Boot 的控制台日志（stdout）；结构化追踪依赖数据库中的 `audit.api_audit_log`（HTTP 审计）与 `audit.business_replay_log`（消息回放），便于按用户/订单聚合排查（详见 `docs/日志与审计.md`）。暂未接入 ELK/Loki 等集中日志系统（可作为后续选做）。