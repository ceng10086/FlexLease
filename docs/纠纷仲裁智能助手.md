...
- `FLEXLEASE_LLM_ENABLED`：`order-service` 的 `application.yml` 默认 `false`（本地不配置时不会调用外部 LLM）；`docker-compose.yml` 中为便于演示默认会把它置为 `true`（可通过 `.env` 覆盖）。
- `FLEXLEASE_LLM_BASE_URL`（默认 `https://api.deepseek.com`）
- `FLEXLEASE_LLM_MODEL`（默认 `deepseek-chat`）
- `FLEXLEASE_LLM_API_KEY`

如不接入外部 LLM（例如无法访问外网/没有 Key），请将 `FLEXLEASE_LLM_ENABLED=false`，并避免触发“生成仲裁建议”按钮；此时相关接口会返回“功能未启用”提示。
