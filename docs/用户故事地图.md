...
- `>=90`（信用优享）：`OrderPreviewResponse` 允许押金减免 30%，并提供“快速审核”通道；支付初始化时基于订单信用快照将平台抽成再减 1 个百分点（最低为 0）。
- `60~89`（标准管控）：照常流程。
- `40~59`（预警）：`order-service` 创建订单时增加 `requiresManualReview` 标记（风险关注，不阻断支付/履约），管理员可在 `frontend/src/views/admin/AdminOrderMonitorView.vue` 中筛选。
- `<40`（限制）：信用等级为 `RESTRICTED` 时将拒绝创建新订单（服务端直接返回 `403`）。
...
| 纠纷升级 | 纠纷抽屉 + 平台裁决 | 管理员在订单监控/纠纷抽屉中完成裁决；纠纷请求支持 `phoneMemo` 记录电话沟通要点 |
...
| 节点 | 证明要求 | 存储设计 | 用户故事 |
| --- | --- | --- | --- |
| 厂商发货 | 3 张照片（外观、序列号、包装）、1 段 15 秒以内开机演示视频 | `order-service` 接收 `/orders/{orderId}/proofs` 上传：元数据写入 `order.order_proof`，文件落在 `flexlease.proof-storage.root`（默认 `/app/storage/order-proofs`） | 厂商在发货抽屉中上传证明，系统按 `contentType/后缀` 识别图片/视频并统计数量，图片会尽力打上“订单号+时间”的水印；上传完成后再允许提交运单 |
