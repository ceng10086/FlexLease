...
| payment_transaction_id | UUID | NULL（关联 `payment.payment_transaction.id`，支付完成后写入） |
...
#### 2.4.2 状态机描述
- `PENDING_PAYMENT`：创建待支付，若超时由调度器自动取消；支付成功进入 `AWAITING_SHIPMENT`。
- `AWAITING_SHIPMENT`：厂商填写物流 → `IN_LEASE`，同时触发库存出库+释放预占。
- `IN_LEASE`：正常履约，可发起续租、退租或买断：
  - 续租：生成 `OrderExtensionRequest`，审批通过后 `extension_count` +1。
  - 退租：先进入 `RETURN_REQUESTED`，厂商接收后切换 `RETURN_IN_PROGRESS`，最终 `COMPLETED`。
  - 买断：进入 `BUYOUT_REQUESTED`，确认后 `BUYOUT_COMPLETED`。
- `COMPLETED`：退租完成或租期结束。
- `CANCELLED`：支付前用户取消 / 管理员强制关闭 / 调度自动取消。
- `EXCEPTION_CLOSED`：用于记录无法恢复的诉求（如支付与库存一致性补偿失败）。
...
> `order.return_request`（换货场景扩展）等表保持在 Backlog，待后续迭代扩展。

#### 2.4.7 `order.rental_contract`
| 字段 | 类型 | 约束 | 说明 |
| ---- | ---- | ---- | ---- |
| id | UUID | PK |
| order_id | UUID | UNIQUE, FK -> order.rental_order.id | 一对一关联的订单 |
| contract_number | VARCHAR(50) | NOT NULL | 合同编号（`LC-日期-订单尾号`） |
| content | TEXT | NOT NULL | 合同正文，按订单动态渲染 |
| status | VARCHAR(20) | NOT NULL (`DRAFT`/`SIGNED`) |
| signature | VARCHAR(255) | NULL | 用户签名快照 |
| signed_by | UUID | NULL | 签署人（必须为订单所属用户） |
| generated_at | TIMESTAMP WITH TIME ZONE | NOT NULL |
| signed_at | TIMESTAMP WITH TIME ZONE | NULL |
| created_at / updated_at | TIMESTAMP WITH TIME ZONE | NOT NULL | 审计信息 |

#### 2.4.8 `order.cart_item`
| 字段 | 类型 | 约束 |
| ---- | ---- | ---- |
| id | UUID | PK |
...
| product_name | VARCHAR(200) | NOT NULL |
| sku_code | VARCHAR(64) | NULL |
| plan_snapshot | TEXT | NULL（固化方案价格 JSON） |
| quantity | INT | NOT NULL |
| unit_rent_amount | NUMERIC(18,2) | NOT NULL |
| unit_deposit_amount | NUMERIC(18,2) | NOT NULL |
| buyout_price | NUMERIC(18,2) | NULL |
...
> 购物车条目会在提交订单并由 `cartItemIds` 引用时自动加载→下单→删除，保持与用户 ID 强一致性。
