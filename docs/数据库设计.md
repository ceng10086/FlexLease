...
订单域（order schema）
 ...
 ├─ rental_contract        租赁合同/履约信息
 ├─ order_event_log        状态变更与审计
 └─ return_request         退租/换货申请
...
| ---- | ---- | ---- |
| id | UUID | PK |
| vendor_id | UUID | FK -> user.org_vendor.id |
| name | VARCHAR(200) | NOT NULL |
| category_code | VARCHAR(100) | NOT NULL |
| description | TEXT | NULL |
| cover_image_url | VARCHAR(255) | NULL |
| status | SMALLINT | NOT NULL (0-草稿 1-待审核 2-已上架 3-已下架) |
...
| plan_type | SMALLINT | NOT NULL (0-普通租赁 1-先租后买 2-以租代售) |
...
| extension_unit | SMALLINT | NULL (1-天 2-周 3-月) |
| extension_price | NUMERIC(18,2) | NULL |
| status | SMALLINT | NOT NULL (0-草稿 1-启用 2-停用) |
| created_at | TIMESTAMP | NOT NULL |
| updated_at | TIMESTAMP | NOT NULL |
...
| status | SMALLINT | NOT NULL (0-停用 1-启用) |
| created_at | TIMESTAMP | NOT NULL |
| updated_at | TIMESTAMP | NOT NULL |
...
| change_type | SMALLINT | NOT NULL (1-入库 2-出库 3-预占 4-释放) |
...
| vendor_id | UUID | FK -> user.org_vendor.id |
| status | SMALLINT | NOT NULL (详见状态机) |
...
#### 2.4.4 `order.rental_contract`
| 字段 | 类型 | 约束 |
| ---- | ---- | ---- |
| id | UUID | PK |
| order_id | UUID | UNIQUE, FK |
| contract_no | VARCHAR(40) | UNIQUE |
| lease_start_at | TIMESTAMP | NOT NULL |
| lease_end_at | TIMESTAMP | NOT NULL |
...
#### 2.4.5 `order.return_request`
| 字段 | 类型 | 约束 |
| ---- | ---- | ---- |
| id | UUID | PK |
| order_id | UUID | FK |
| request_type | SMALLINT | NOT NULL (1-退租 2-换货) |
| status | SMALLINT | NOT NULL (0-申请中 1-处理中 2-完成 3-拒绝) |
| reason | TEXT | NULL |
| logistics_tracking | VARCHAR(50) | NULL |
| created_at | TIMESTAMP | NOT NULL |
| updated_at | TIMESTAMP | NOT NULL |
